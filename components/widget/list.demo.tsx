...
export function ListBasicDemo() {
  ...
  return (
    <div className=" border rounded-lg">
      <WidgetState loading={loading} error={error} empty={!loading && !error && items.length === 0} onRetry={refresh}>
...
export function ListResizableDemo() {
  ...
  return (
    <ResizableContainer background="primary" shadow="soft" padding="comfortable">
      <WidgetState loading={loading} error={error} empty={!loading && !error && items.length === 0} onRetry={refresh}>
        <Scroll>
          <List title="示例列表" items={items} />
        </Scroll>
